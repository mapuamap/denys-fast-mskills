...
(function (g) {
  ...
  var BASE = g.MW_BASE || "https://cdn.jsdelivr.net/gh/mapuamap/denys-fast-mskills@v2.5.0/docs/";
  ...
  function injectStyle() {
    ...
    var css =
      ...
      '.mw-card{font-family:ui-monospace,"SF Mono",Menlo,Consolas,monospace;max-width:560px;' +
        'background:' + INK + ';border:0.5px solid ' + LINE + ';border-radius:14px;' +
        'padding:13px 16px 11px;color:' + TEXT + ';animation:mw-in .45s ease both;position:relative;overflow:hidden}' +
  ...
  function build(root, d) {
    var tk = d.tokens || {};
    var tools = d.tools_by_name || {};
    var toolStr = Object.keys(tools).map(function (k) { return esc(k) + "×" + tools[k]; }).join(" · ");
    ...
    root.innerHTML =
      ...
      '<div class="mw-foot">' +
        ...
        (toolStr ? '<span class="sep">|</span><span>' + toolStr + '</span>' : '') +
      '</div>';
