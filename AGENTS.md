...
| **Hooks** (`hooks/`) | `hooks.json` (Stop) → two hooks: `plan_completion_driver.py` (m_plan autodrive — blocks stopping until `09_verification.md` is clean, armed via `.m_plan/<slug>/.run.json`) + `turn_summary_widget.py` (end-of-turn one-line stats summary + silent `.m_verify/` ledger feed on code-change turns; the branded logo widget was removed in v2.11.0). Plugin-native, fail-open. |
...
- **Feed gate:** the hook only appends on turns where code changed (`CODE_TOOLS` = Edit/Write/MultiEdit/NotebookEdit present in `tools_by_name`). The append is a *silent* instruction (file IO only, nothing in chat) — the hook's `decision:block` reason on code-change turns contains only this ledger instruction. (Before v2.11.0 this was a 4th step after a branded widget + "what's next"/"task" notes; that widget was removed.)
...
### m_plan autodrive — completion-driver Stop hook (deterministic /goal)
`hooks/plan_completion_driver.py` turns `09_verification.md` (the completion oracle) into an **enforced gate** so an `m_plan` / `m_plan_implement` run drives itself to done without the user manually invoking `/goal`.
- **Armed per-run, never global.** The skill writes `.m_plan/<slug>/.run.json` (`mode:autodrive`, `max_turns`, counters) before the Phase B walk and deletes it when it reports. **With no armed `.run.json` the hook is completely silent** — ordinary sessions are untouched. This is why it's safe as a global Stop hook.
- **Gate:** while armed, it counts open `[ ]` rows in `09`; >0 → `decision:block` with a focused "do the next cited check" reason; 0 → disarm + allow stop. `[!]` blockers and `[~]` skips do **not** block (honest partial).
- **Loop safety is ours** (Claude Code has no built-in turn cap): turn counter vs `max_turns` (clamped `[12,80]`), no-progress streak (`NO_PROGRESS_LIMIT=2`), and **user-takeover detection** — if a new human prompt appears after arming (user interrupted and typed) or the last prompt is a stop-word, it disarms. Any error / missing oracle → fail-open exit 0.
- **Hook coexistence:** both Stop hooks fire; the driver runs first and its reason tells the agent to skip the end-of-turn summary for that turn (cosmetic only). The summary renders normally once the run disarms. Driver order is first in `hooks.json`.
- **Contract with the skills:** `m_plan_implement` Phase B arms / Phase C disarms; `m_plan` Phase 2 arms / Phase 3 disarms. The `.run.json` schema is shared between the hook and both skills — change one, change the others. Hook-logic changes need a normal plugin release (bump + reinstall).

### End-of-turn summary — one-line stats only (widget removed in v2.11.0)
The branded in-chat widget (logo card, "what's next"/"task" notes, the `show_widget` render step) was **removed at the owner's request** in v2.11.0. What `turn_summary_widget.py` does now:
- Every non-empty turn → a lightweight one-line ANSI `systemMessage` (project, token/cost/time stats) via `one_line_summary()`. No `decision:block`, no widget, no extra chat.
- Real-work turns that **changed code** → `decision:block` whose reason is *only* the silent `.m_verify` ledger instruction (`build_ledger_reason()`), then the agent stops. This is the sole remaining use of the one-more-turn mechanism.
...
- **Legacy assets** `docs/widget/m_widget.js` and `docs/widget/index.html` are now **unreferenced** by the hook (no `WIDGET_URL`, no jsDelivr fetch). Left in the repo for history / possible reuse; safe to delete in a later cleanup. The Russian-language render-instruction note in *Conventions* now applies only to `build_ledger_reason()`.

## Conventions

- **Language:** everything user-facing is **English** (READMEs, SKILL/agent bodies, landing page). (Originals were partly Russian; that was translated on first release — keep new content English.) **Deliberate exception:** the Stop-hook *ledger instruction* in `hooks/turn_summary_widget.py` (`build_ledger_reason()`) is Russian — that's the owner's personal hook text; keep new hook-instruction text consistent with that file. The `.m_verify` ledger *field names* stay English (parsing contract); entry titles/notes follow the working language.
