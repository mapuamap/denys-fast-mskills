...
| **Hooks** (`hooks/`) | `hooks.json` (Stop) → one hook: `turn_summary_widget.py` (end-of-turn one-line stats summary + silent `.m_verify/` ledger feed on code-change turns; the branded logo widget was removed in v2.11.0). The `plan_completion_driver.py` autodrive hook was removed in v2.12.0. Plugin-native, fail-open. |
