...
- **Feed gate:** the hook only appends on turns where code changed (`CODE_TOOLS` = Edit/Write/MultiEdit/NotebookEdit present in `tools_by_name`). The append is a *silent* instruction (file IO only, nothing in chat) — since v2.14.0 it is the **entire** `decision:block` reason (`build_ledger_reason()`), no widget and no summary text wrapping it.
...
### End-of-turn summary — stats line + silent ledger only (widget gone v2.11.0, text gone v2.14.0)
Two end-of-turn features were removed at the owner's request: the branded **widget** (logo card + `show_widget` render step) in v2.11.0, and the plain-text **«Что дальше»/«Задача» summary blocks** in v2.14.0 (the owner disliked the "what's done / what's next" recap; it had briefly been restored in v2.13.0). What `turn_summary_widget.py` does now:
- Every non-empty turn → a lightweight one-line ANSI `systemMessage` (project, token/cost/time stats) via `one_line_summary()`. No `decision:block`.
- Code-change turns (`tools_total >= MIN_TOOLS_FOR_WIDGET` **and** a `CODE_TOOLS` edit present) → `decision:block` whose reason is *only* the silent `.m_verify` ledger instruction (`build_ledger_reason()`), then the agent stops. This is the sole remaining `decision:block`.
- Every other turn → one-liner only, no block.
...
- **Legacy assets** `docs/widget/m_widget.js` and `docs/widget/index.html` are **unreferenced** by the hook (no `WIDGET_URL`, no jsDelivr fetch) — left for history, safe to delete later. The env knob is still named `CLAUDE_WIDGET_MIN_TOOLS` / `MIN_TOOLS_FOR_WIDGET` for back-compat.
