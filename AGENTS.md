...
| **Hooks** (`hooks/`) | `hooks.json` (Stop) → two hooks: `plan_completion_driver.py` (m_plan autodrive — blocks stopping until `09_verification.md` is clean, armed via `.m_plan/<slug>/.run.json`) + `turn_summary_widget.py` (end-of-turn stats widget + silent `.m_verify/` ledger feed on code-change turns). Plugin-native, fail-open. |
...
- **Widget coexistence (why the widget hook is untouched):** both Stop hooks fire; the driver runs first and its reason tells the agent to skip the end-of-turn widget for that turn (cosmetic only). The widget renders normally once the run disarms. Driver order is first in `hooks.json`.
